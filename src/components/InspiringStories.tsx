...
export function InspiringStories() {
  ...
  const stories = allStories ? allStories.slice(0, 5) : [];
    ...
    <section className="py-24 md:py-32 bg-[#F3EFE7] text-[#0B2545] overflow-hidden">
      <div className="container mx-auto px-4 sm:px-6 max-w-6xl">
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-gray-600 text-lg">New stories of strength and hope will be shared here soon.</p>
          </div>
        ) : (
          <>
                  ...
                  >
                    ...
                    >
                      <span className="rotate-180 [writing-mode:vertical-rl] font-bold text-white text-lg md:text-xl tracking-wider whitespace-nowrap drop-shadow-md">
                        {(story.title || story.name || '').split(',')[0]}
                      </span>
                    </div>
                    ...
                    <div 
                      ...
                      `}
                    >
                      <h3 className="font-black text-3xl md:text-4xl text-white mb-2 tracking-tighter">{story.title || story.name}</h3>
                      ...
                      <div className="bg-white/10 backdrop-blur-md rounded-[2rem] p-6 border border-white/20 shadow-xl">
                        <div className="w-full bg-black/20 h-2 rounded-full overflow-hidden mb-3">
                          <div 
                            ...
                            style={{ width: isActive ? `${story.progress || 0}%` : '0%' }}
                          ></div>
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          >
                            {(story.progress || 0) >= 100 ? 'Fully Funded' : `Support ${(story.title || story.name || '').split(',')[0]}`}
                          </button>
                          <button 
                            onClick={(e) => {
                              e.stopPropagation();
